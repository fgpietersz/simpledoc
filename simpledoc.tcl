...
set header {<html><head><title>$infile</title>$style</head>}
...
proc text2html {x} {
	...
	foreach i $x {
		...
		if {[string index $i 0] == {=}} {
			...
			append out "<h$mlevel><a name=\"[incr anchor]\"></a>[string range $i $level end]</h$mlevel>"
			append index "<h$mlevel><a href=\"#$anchor\">[string range $i $level end]</a></h$mlevel>"
			set mode para
			continue
		}
		if {$mode == {break}} {
...
#Jeff Hobb's tcl2html. Adapted from original script that was BSD licensed.
proc tcl2html {data} {
...
#==Copyright
#Copyright (c) Graeme Pietersz. Licensed under the GPL version 2 or later. Incorporates BSD licensed code by Jeff Hobbs.
